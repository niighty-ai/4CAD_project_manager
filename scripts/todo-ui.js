...
function _todoTaskRowHtml(task, isSub) {
  ...
  return `
    <div class="todo-task-row ${subClass} ${doneClass}" data-task-id="${task.id}"
      ...
      <div class="todo-task-content" onclick="_todoOpenModal('${task.id}')">
        <div class="todo-task-title">${_esc(task.title)}</div>
        ${meta ? `<div class="todo-task-meta">${meta}</div>` : ''}
      </div>
      <div class="todo-task-actions">
        <div class="todo-task-action-btn" title="Modifier" onclick="event.stopPropagation();_todoOpenModal('${task.id}')">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
          </svg>
