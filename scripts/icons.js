/* ═══════════════════════════════════════════════════════════════
   icons.js — Bibliothèque d'icônes SVG Lucide centralisée
   Contraintes : stroke-width="2", 24×24, stroke="currentColor"
   Fichiers source : /assets/icons/*.svg
   ═══════════════════════════════════════════════════════════════ */
...
const ICONS = {
  ...
  'users':
    '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 1-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>',
...
/* ── Met à jour le bouton thème selon l'état courant ────────────
   isDark = true  → on est en mode sombre  → affiche "☀ Clair"  (soleil)
   isDark = false → on est en mode clair   → affiche "🌙 Sombre" (lune)
   ─────────────────────────────────────────────────────────────── */
function _setBtnTheme(isDark) {
  const btn = document.getElementById('btnTheme');
  if (!btn) return;
  btn.innerHTML = isDark
    ? ICONS.sun  + ' <span class="btn-theme-lbl">Clair</span>'
    : ICONS.moon + ' <span class="btn-theme-lbl">Sombre</span>';
}

/* ── Initialisation au chargement (applique l'état sauvegardé) ── */
(function () {
